...
import {
    Upload, Image, Code, Key, Zap, Shield, Globe,
    Copy, Check, ChevronRight, Terminal, FileImage,
    ArrowRight, Sparkles, Clock, HardDrive, RefreshCw
} from 'lucide-react';
...
const ImageUploadAPI = () => {
    ...
    const fetchApiKey = async () => {
        try {
            ...
            const uploadKey = res.data.apiKeys?.find(k =>
                k.tool?.slug === 'image-upload' || k.tool?.category === 'Storage'
            );
            if (uploadKey) setApiKey(uploadKey);
        } catch (err) {
    ...
    const generateApiKey = async () => {
        ...
        setLoading(true);
        try {
            // Find or create the image-upload tool
            const toolsRes = await api.get('/saas');
            let uploadTool = toolsRes.data.tools?.find(t => t.slug === 'image-upload');
            ...
            // Generate API key
            const res = await api.post(`/saas/${uploadTool.id}/generate-key`, {
