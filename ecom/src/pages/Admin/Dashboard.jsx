...
const Dashboard = () => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const [productsRes, ordersRes, docsRes, toolsRes, usersRes, userStatsRes] = await Promise.allSettled([
                api.get('/products'),
                api.get('/orders'),
                api.get('/docs'),
                api.get('/saas'),
            ...
            if (ordersRes.status === 'fulfilled') {
                const ords = ordersRes.value.data.orders || [];
                setOrders(ords);
                // Calculate stats
                const revenue = ords.filter(o => o.isPaid).reduce((sum, o) => sum + (o.totalPrice || 0), 0);
                setStats(prev => ({
                    ...prev,
            ...
            if (userStatsRes.status === 'fulfilled') {
                setUserStats(userStatsRes.value.data.stats || {});
            }
        } catch (error) {
    ...
    const updateUserSubscription = async (userId, planName) => {
        try {
            ...
            setAllUsers(allUsers.map(u => u.id === userId ? { ...u, subscription: { planName, status: 'ACTIVE' } } : u));
