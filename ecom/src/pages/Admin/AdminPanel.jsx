...
import {
    ...
    Plus, Edit, Trash2, Search, RefreshCw, Crown, Shield, Eye,
    TrendingUp, DollarSign, BarChart3, Download, Filter, ChevronRight,
    Settings, BookOpen, Smartphone, Code, Key, ArrowUpRight
} from 'lucide-react';
...
// Overview Tab
const OverviewTab = ({ stats, recentOrders, recentUsers, onRefresh }) => (
    <div>
                ...
                {recentOrders.length === 0 ? (
                    ...
                    <div className="space-y-3">
                        {recentOrders.slice(0, 5).map(order => (
                            <div key={order.id} className="flex items-center justify-between py-2 border-b border-[var(--border-secondary)] last:border-0">
                                <div>
                ...
                {recentUsers.length === 0 ? (
                    ...
                    <div className="space-y-3">
                        {recentUsers.slice(0, 5).map(u => (
                            <div key={u.id} className="flex items-center gap-3 py-2 border-b border-[var(--border-secondary)] last:border-0">
                    ...
                    {products.map(product => (
                        <div key={product.id} className="card flex items-center gap-4">
                            ...
                            <div className="flex gap-2">
                                <Link to={`/admin/product/${product.id}/edit`} className="p-2 hover:bg-[var(--bg-tertiary)] rounded-lg">
...
const BlogsTab = () => {
    ...
    const fetchBlogs = async () => {
        try {
            ...
            setBlogs(res.data.docs || res.data || []);
        } catch (err) {
                    ...
                    {blogs.map(blog => (
                        <div key={blog.id || blog._id} className="card flex items-center gap-4">
                            ...
                            <div className="flex gap-2">
                                ...
                                <button onClick={() => handleDelete(blog.id || blog._id)} className="p-2 hover:bg-red-50 rounded-lg">
                                    <Trash2 className="w-4 h-4 text-red-500" />
                                </button>
                            </div>
                        </div>
